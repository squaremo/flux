...
func (s *Server) UpdateImages(instID service.InstanceID, spec update.ReleaseSpec) (job.ID, error) {
	inst, err := s.instancer.Get(instID)
	if err != nil {
...
func (s *Server) UpdatePolicies(instID service.InstanceID, updates policy.Updates) (job.ID, error) {
	inst, err := s.instancer.Get(instID)
	if err != nil {
		return "", errors.Wrapf(err, "getting instance "+string(instID))
	}
...
func (s *Server) LogEvent(instID service.InstanceID, event history.Event) error {
...
func (s *Server) History(inst service.InstanceID, spec update.ServiceSpec, before time.Time, limit int64) (res []history.Entry, err error) {
...
func (s *Server) SetConfig(instID service.InstanceID, updates service.UnsafeInstanceConfig) error {
...
func applyConfigUpdates(updates service.UnsafeInstanceConfig) instance.UpdateFunc {
...
// service iface
func (s *Server) GenerateDeployKey(instID service.InstanceID) error {
	// Generate new key
	...
	// Set new config
	return s.config.UpdateConfig(instID, applyConfigUpdates(service.UnsafeInstanceConfig(cfg)))
}
