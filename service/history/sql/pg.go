...
func (db *pgDB) EventsForService(inst service.InstanceID, service flux.ServiceID, before time.Time, limit int64) ([]history.Event, error) {
	q := db.eventsQuery().
...
func (db *pgDB) AllEvents(inst service.InstanceID, before time.Time, limit int64) ([]history.Event, error) {
	q := db.eventsQuery().
		Where("instance_id = ?", string(inst)).
		Where("started_at < ?", before)
	if limit >= 0 {
